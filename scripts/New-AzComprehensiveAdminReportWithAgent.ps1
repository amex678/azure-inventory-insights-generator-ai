<#
.SYNOPSIS
    AI API 経由で Azure 棚卸しデータから
    comprehensive-report.html を生成する。

.DESCRIPTION
    - 入力 JSON から compact data を構築
    - prompt seed と結合して AI API に HTML 生成を依頼
    - 生成結果を最小限検証して保存
...
- UTF-8 前提
- フッターに「AI-Generated Report」と「Generated by GitHub Actions + AI API」を明記
    - フッター表示例: 「AI-Generated Report | Generated by GitHub Actions + AI API | 2026-07-10」
...
try {
    ...
    if (Test-Path $fallbackScript) {
        ...
        ($fallbackEvidence | ConvertTo-Json -Depth 8) | Set-Content -Path $EvidencePath -Encoding utf8
    } else {
